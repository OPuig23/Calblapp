...
export default function CalendarHourGrid({ events, onEventClick }: CalendarHourGridProps) {
  ...
  return (
    <div className="overflow-x-auto pb-4">
      ...
      <div className="grid grid-cols-8 border-t border-gray-200">
        ...
        <div className="flex flex-col border-r border-gray-200">
          {hours.map(h => (
            <div key={h} className="h-10 text-right pr-2 text-xs text-gray-500 leading-10">
        ...
        {/* Columnes dies */}
        {dayKeys.map((key, idx) => (
          <div key={key} className="flex flex-col relative border-r border-gray-100 min-w-[120px]">
    ...
    treballadors: (ev as any).treballadors,
              ...
              const color = colorMap[ev.status as keyof typeof colorMap] || 'bg-gray-300 border-gray-400';
              return (
                <div
                  key={ev.id}
                ...
                >
                  ...
                  <span className="text-[10px] opacity-80">{format(start, 'HH:mm')} - {format(end, 'HH:mm')}</span>
                </div>
              );
            })}
