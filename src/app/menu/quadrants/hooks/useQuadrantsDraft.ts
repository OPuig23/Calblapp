...
export interface Draft {
  ...
  conductors?: any[]      // string[] o [{id,name}]
  treballadors?: any[]    // string[] o [{id,name}]
  updatedAt: string
}
...
function anyArrayToNames(arr?: any[]): string[] {
...
export function useQuadrantsDraft(params?: { department?: string }) {
  ...
  // Nota: el servidor ja força el departament segons rol; per Admin/Direcció podràs passar-lo aquí més endavant
  ...
  const filteredDrafts = useMemo(() => {
    ...
    return allDrafts.filter((d) => {
      const w = anyArrayToNames(d.treballadors).some((n) => n.toLowerCase().includes(pf))
      const c = anyArrayToNames(d.conductors).some((n) => n.toLowerCase().includes(pf))
  ...
  const groupedByDate = useMemo(() => {
    ...
    // Retornem com a array ordenada de dies
