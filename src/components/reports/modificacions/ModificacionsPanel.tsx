...
const defaultOptions: FilterOptions = { events: [], lines: [], departments: [], categories: [], commercials: [], importances: [] }

export function ModificacionsPanel() {
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      ...
      const res = await fetch(`/api/reports/modificacions?${params.toString()}`, { cache: 'no-store' })
  ...
  return (
    <div className="flex flex-col gap-4">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
        <KpiTotal value={summary?.total || 0} />
        <KpiLast72 value={summary?.last72 || 0} total={summary?.total || 0} />
